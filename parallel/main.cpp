...
bool Party::colorExistByDist(int c, int x, int y, int d)
{
	int cornerY = (y-d < 0)?0:(y-d);
	int cornerX = (x-d < 0)?0:(x-d);
	int lenY = (y+d+1 < size)?(y+d+1):size;
	int lenX = (x+d+1 < size)?(x+d+1):size;
...
{
	int d = MAX((MAX(x, y)), (MAX(size-x-1, size-y-1)));
